...
2. Para acceso público sin registro, define `AUTH_MODE=open`, `SUPABASE_URL` y `SUPABASE_SERVICE_ROLE_KEY` únicamente en el API. El API usa el observador técnico `guest-observer`, por lo que todas las personas comparten esa identidad; esta opción es intencionalmente no autenticada.
3. Para exigir cuentas de Supabase Auth, define `AUTH_MODE=supabase`, `SUPABASE_URL`, `SUPABASE_ANON_KEY` y `SUPABASE_SERVICE_ROLE_KEY` únicamente en el API.
